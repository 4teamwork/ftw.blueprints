...
class TestParseXMLString(TestCase):
    ...
    def test_functionality_with_unicode(self):
        self.failUnless(self.handler.parse_xml_string(u'<j\xe4mes></j\xe4mes>'))

    def test_functionality_with_utf_8(self):
        self.failUnless(self.handler.parse_xml_string(
            '<j\xc3\xa4mes></j\xc3\xa4mes>'))

    def test_error_message_with_invalid_xml_string(self):
        self.assertRaises(ExpatError, self.handler.parse_xml_string, 'bad_xml')
...
class TestGetElementValue(TestCase):
    ...
    def test_emty_string_when_no_elements_with_value(self):
...
class TestGetElementAttribute(TestCase):
    ...
    def test_emty_string_when_attribute_doesnt_exist(self):

        value = self.handler.get_element_attribute_value(
            TEST_XML, 'bond', 'nothing')
        self.assertEquals('', value)

    def test_emty_string_when_element_doenst_exist(self):
        value = self.handler.get_element_attribute_value(
